...
{
    /// <summary>
    /// Interaction logic for BizTalkAdminOperations.xaml
    /// </summary>
    public partial class BizTalkAdminOperations : UserControl
    {
       

        void AllowUIToUpdate() //Equivalent to refresh in winform
        {
        ...
        private static TextPointer GetPoint(TextPointer start, int x) //Utility for WPF select
        {
            var ret = start;
            var i = 0;
            while (i < x && ret != null)
        ...
        {
            Loaded+=BizTalkAdminOperations_Load;
            strWebSite = "n";
            strAppPool = "n";
            strCertificate = "n";
            strHandlers = "n";
                        ...
                        try
                        {
                            if (strFoldersToCopyNoFiles != string.Empty)
                            {
                                ...
                                foreach (string folderPath in arrFoldersToCopyNoFiles)
                                {
                                    ...
                                    string driveInfo = System.IO.Path.GetPathRoot(folderPathTrimed);
            ...
            try
            {
                ...
                if (files.Length != 0)
                {
                    foreach (string file in files)
                    {
                        ...
                        if (result == 0)
                            LogShortSuccessMsg("Success: Imported Host Settings on " + System.IO.Path.GetFileNameWithoutExtension(file).Split('_')[1].Split('_')[0] + " Server.");
                        else
                            LogShortErrorMsg("Failed: Importing Host Settings on" + System.IO.Path.GetFileNameWithoutExtension(file).Split('_')[1].Split('_')[0] + " Server.");
                    }
                }
        ...
        private void LogShortSuccessMsg(string pShortMsg)
        {
            ...
            //int startIndex = richTextBoxLogs.Selection.Text.Length;
            //richTextBoxLogs.AppendText(pShortMsg);
            //richTextBoxLogs.SelectAll();
            //int endIndex = richTextBoxLogs.Selection.Text.Length;

            //var start = richTextBoxLogs.Document.ContentStart;
            //var startPos = GetPoint(start, startIndex);
            //var endPos = GetPoint(start, startIndex + (endIndex - startIndex));

            //var converter = new System.Windows.Media.BrushConverter();
            //var DarkGreen = (System.Windows.Media.Brush)converter.ConvertFromString("#FF3D6E14");
            //richTextBoxLogs.Selection.Select(startPos, endPos);
            //richTextBoxLogs.Foreground = DarkGreen;
            TextRange tr = new TextRange(richTextBoxLogs.Document.ContentEnd,richTextBoxLogs.Document.ContentEnd);
            tr.Text = pShortMsg;
        ...
        private void LogShortErrorMsg(string pShortErrorMsg) //UI
        {
            ...
            //richTextBoxLogs.Selection.Select(startPos, endPos);
            //var converter = new System.Windows.Media.BrushConverter();
            //var Red = (System.Windows.Media.Brush)converter.ConvertFromString("#FFE43941");
            //richTextBoxLogs.Foreground = Red;
            TextRange tr = new TextRange(richTextBoxLogs.Document.ContentEnd, richTextBoxLogs.Document.ContentEnd);
            tr.Text = pShortErrorMsg;
        ...
        private void LogException(Exception ex)  //UI FILLE
        {
            ...
            //richTextBoxLogs.Selection.Select(startPos, endPos);
            //var converter = new System.Windows.Media.BrushConverter();
            //var DarkRed = (System.Windows.Media.Brush)converter.ConvertFromString("#FF93070E");
            //richTextBoxLogs.Foreground = DarkRed;
            TextRange tr = new TextRange(richTextBoxLogs.Document.ContentEnd, richTextBoxLogs.Document.ContentEnd);
            tr.Text = "Exception Message:  " + ex.Message.ToString();
            tr.ApplyPropertyValue(TextElement.ForegroundProperty, System.Windows.Media.Brushes.DarkRed);
        ...
        private void btnExportWebSites_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < srcWebSites.Length; i++)
                {
                    ...
                    string webSiteFileName = System.IO.Path.GetFileNameWithoutExtension(webSiteFilePath);
                            ...
                            for (int iFile = 0; iFile < strFiles.Length; iFile++)
                            {
                                ...
                                try
                                {
                                    ...
                                    if (storeNam == StoreName.My)
                                    {
                                        ...
                                        {
                                            if (System.IO.Path.GetExtension(strFiles[iFile]) == ".PFX")
                                            {
                                ...
                                else
                                {
                                    ...
                                    foreach (string filePath in findDLL)
                                    {
                                        ...
                                        try
                                        {
                                            ...
                                            dir = asmPath + "\\" + System.IO.Path.GetFileNameWithoutExtension(filePath) + "_" + ver;
                                            ...
                                            File.Copy(filePath, dir + "\\" + System.IO.Path.GetFileName(filePath), true);
                                            LogInfoInLogFile("Assembly copied: " + System.IO.Path.GetFileName(filePath) + ", " + ver);
                                            // writer.WriteLine(Path.GetFileNameWithoutExtension(filePath) + "_" + ver);
                                            bizTalkDll.Add(System.IO.Path.GetFileNameWithoutExtension(filePath) + "_" + ver);
                                        }
                                        catch (Exception ex)
                                        { LogException(ex); }
                                    }
                                ...
                                foreach (string filePath in customDll)
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        if (!(string.IsNullOrEmpty(filePath)) && !(string.IsNullOrWhiteSpace(filePath)))
                                        {
                                            ...
                                            dir = customDllPath + "\\" + System.IO.Path.GetFileNameWithoutExtension(filePath) + "_" + CustomDllVer;
                                            ...
                                            //dir = ConvertPathToUncPath(dir);
                                            File.Copy(filePath, dir + "\\" + System.IO.Path.GetFileName(filePath), true);
                                            writer.WriteLine(System.IO.Path.GetFileNameWithoutExtension(filePath) + "_" + CustomDllVer);
                                            LogInfoInLogFile("Assembly copied: " + System.IO.Path.GetFileName(filePath) + ", " + CustomDllVer);
                                        }
                                    }
                                    catch (Exception ex)
                                    {
                                        LogShortErrorMsg("Assembly copy failed: " + System.IO.Path.GetFileName(filePath) + ", " + CustomDllVer);
                                        LogException(ex);
                                    }
                                }
                            }
                        ...
                        using (StreamWriter writer = new StreamWriter(xmlPath + @"\DstCustomAssemblyList.txt", false))
                        {
                            foreach (string filePathCusDll in customDllDst)
                            {
                                if (filePathCusDll != string.Empty)
                                {
                                    ...
                                    writer.WriteLine(System.IO.Path.GetFileNameWithoutExtension(filePathCusDll) + "_" + CustomDllVer);
                                }
        ...
        {
            if (rbWebsiteYes.IsChecked == true)
            {
        ...
        {
            if (rbAppPoolYes.IsChecked == true)
            {
        ...
        private void rbCertificateYes_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (rbCertificateYes.IsChecked == true)
            {
        ...
        {
            if (rbHostInstanceYes.IsChecked == true)
            {
        ...
        {
            if (rbHandlersYes.IsChecked == true)
            {
        ...
        {
            if (rbGlobalPartyBindingYes.IsChecked == true)
            {
        ...
        {
            if (rbBizTalkAssembliesYes.IsChecked == true)
            {
        ...
        {
            if (rbBizTalkAppYes.IsChecked == true)
            {
        ...
        {
            if (rbFileCopyYes.IsChecked == true)
            {
        ...
        {
            if (rbApp.IsChecked == true)  //if App
            {
                strServerType = "App";
                panel4.IsEnabled = false;
                panel5.IsEnabled = false;
                panel8.IsEnabled = false;
                panel9.IsEnabled = false;
                panel10.IsEnabled = false;
                cmbBoxServerDst.Visibility = Visibility.Hidden;
                cmbBoxServerSrc.Visibility = Visibility.Hidden;
                label2.Content = "Server  (Source)";
                label15.Content = "Server  (Destination)";
            ...
            else  //BizTalk
            {
                ...
                panel5.IsEnabled = true;
                panel8.IsEnabled = true;
                panel9.IsEnabled = true;
                panel10.IsEnabled = true;
                cmbBoxServerDst.Visibility = Visibility.Visible;
                cmbBoxServerSrc.Visibility = Visibility.Visible;
                label2.Content = "BizTalk Mgmt Grp (Src)";
                label15.Content = "BizTalk Mgmt Grp (Dst)";
        ...
        {
            if (rbServicesYes.IsChecked == true)
            {
            ...
            try
            {
                ...
                else
                {
                    if (strFileCopy == strPerformOperationYes || strAppPool == strPerformOperationYes || strWebSite == strPerformOperationYes || strCertificate == strPerformOperationYes || strHostInstance == strPerformOperationYes || strHandlers == strPerformOperationYes || strBizTalkApp == strPerformOperationYes || strGlobalPartyBinding == strPerformOperationYes || strBizTalkAssemblies == strPerformOperationYes || strBam == strPerformOperationYes || strServices == strPerformOperationYes)
                    {
                        ...
                        {
                            panelLoginDialog.Visibility = Visibility.Visible;
                        ...
                        rbFileCopyNo.IsChecked = true;
                        rbAppPoolNo.IsChecked = true;
                        rbBizTalkAppNo.IsChecked = true;
                        rbBizTalkAssembliesNo.IsChecked = true;
                        rbCertificateNo.IsChecked = true;
                        rbGlobalPartyBindingNo.IsChecked = true;
                        rbHandlersNo.IsChecked = true;
                        rbHostInstanceNo.IsChecked = true;
                        rbWebsiteNo.IsChecked = true;
                        rbBamNo.IsChecked = true;
                        rbServicesNo.IsChecked = true;
                        EnableControls(true);
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            {
                label18.Content = "UserName or Password missing.";
            ...
            else
            {
                if (loginOperationName == "useraccount") //for remote WMI, remote BizTalkApp 
                {
                    ...
                    strPassword = txtPassword.Password.Trim();
                    strDomain = txtUserName.Text.Split(chrSep)[0];
                }
                else //Host
                {
                    strUserNameForHost = txtUserName.Text.Trim();
                    strPasswordForHost = txtPassword.Password.Trim();
                }

                panelLoginDialog.Visibility = Visibility.Hidden;
        ...
        private void txtConnectionString_Validating(object sender, RoutedEventArgs e)
        {
        ...
        private void txtConnectionStringDst_Validating(object sender, RoutedEventArgs e)
        {
        ...
        private void btnSettings_Click(object sender, EventArgs e)
        {
            ...
            panel11.Children.Clear();
            panel11.Children.Add(settingsPage);
        }
        #endregion
        ...
        private void EnableControls(bool pTrueFalse)
        {
            ...
            {
                panel12.IsEnabled = pTrueFalse;
                panel13.IsEnabled = pTrueFalse;
                btnExportOperations.IsEnabled = pTrueFalse;
                btnImportOperations.IsEnabled = pTrueFalse;
                txtConnectionString.IsEnabled = pTrueFalse;
                txtConnectionStringDst.IsEnabled = pTrueFalse;
                panel1.IsEnabled = pTrueFalse;
                panel2.IsEnabled = pTrueFalse;
                panel3.IsEnabled = pTrueFalse;
                panel6.IsEnabled = pTrueFalse;
                panel7.IsEnabled = pTrueFalse;
                panel11.IsEnabled = pTrueFalse;
                panel14.IsEnabled = pTrueFalse;
            }
            else if (strServerType == "App" && pTrueFalse == false) //App
            {
                panel4.IsEnabled = pTrueFalse;
                panel5.IsEnabled = pTrueFalse;
                panel8.IsEnabled = pTrueFalse;
                panel9.IsEnabled = pTrueFalse;
                panel10.IsEnabled = pTrueFalse;
            ...
            else if (strServerType == "BizTalk")
            {
                ...
                panel9.IsEnabled = pTrueFalse;
                panel10.IsEnabled = pTrueFalse;
                cmbBoxServerDst.IsEnabled = pTrueFalse;
                cmbBoxServerSrc.IsEnabled = pTrueFalse;
            }
        }
        ...
        public void UpdateSettings()
        {
            ...
            string configFile = System.IO.Path.Combine(configPath, "TpmMigration.exe.config");
        ...
        private void ImportSSOApps()
        {
            ...
            try
            {
                ...
                if (files.Length > 0)
                {
                    for (int i = 0; i < files.Length; i++)
                    {
                        string appName = System.IO.Path.GetFileNameWithoutExtension(files[i]).Split('_')[1].Split('_')[0];
                        try
                        {
